...
def main(args: List[str]):
    ...
    parser.add_argument(
        ...
        help="Consider every revision from REV to `--last_revision`")
    parser.add_argument(
        "--test_limit", "-n", type=int, default=None, metavar="N",
        help="Perform no more than N tests; exit with nonzero code if that "
             "wasn't enough to fully convict any particular change.")
